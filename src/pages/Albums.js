...
//const socket = io(`${process.env.REACT_APP_SERVER_BASE_URL}`,{withCredentials: true});
...
export default function Albums() {
  ...
  const handleShare = async (albumId) => {
    try {
      const users = await axios.get(`${process.env.REACT_APP_SERVER_BASE_URL}/v1/users`, {
        params: { currentEmail: user.email },
      });
      setSharedUser(users.data);
      console.log(users);
  ...
  const filterAlbums = albums?.filter((val) => val.ownerId === user.id);
  return (
    <>
    <Header/>
      <div
